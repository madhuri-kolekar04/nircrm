    ...
    <div class="content">
        @switch($notificationType)
            @case('late_check_in')
                <div class="alert alert-warning">
                    ...
                    <p><strong>{{ $user->name }}</strong> has checked in late today.</p>
                </div>
                @break
            ...
            @case('early_checkout')
                <div class="alert alert-warning">
                    ...
                    <p><strong>{{ $user->name }}</strong> has checked out early today.</p>
                </div>
                @break

            @case('marked_by_admin')
                <div class="alert alert-info">
                    <h3>📝 Attendance Marked</h3>
                    <p>Your attendance for <strong>{{ $attendance->date->format('d M Y') }}</strong> has been marked by the administrator.</p>
                </div>
                @break
            ...
            @case('absent')
                <div class="alert alert-warning">
                    ...
                    <p><strong>{{ $user->name }}</strong> was marked absent today.</p>
                </div>
                @break
            ...
            @case('daily_summary')
                <div class="alert alert-success">
                    ...
                    <p>Here's the attendance summary for <strong>{{ $attendance->date->format('d M Y') }}</strong>.</p>
                </div>
                @break
        ...
        <div class="details">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value">{{ $user->employeeID ?? 'N/A' }}</span>
            </div>
            ...
            <div class="detail-row">
                ...
                <span class="detail-value">{{ $attendance->date->format('d M Y') }}</span>
            </div>
            ...
            @if($attendance->working_hours)
                <div class="detail-row">
                    ...
                    <span class="detail-value">{{ number_format($attendance->working_hours, 2) }} hours</span>
                </div>
            @endif
            ...
            @if($attendance->overtime_hours > 0)
                <div class="detail-row">
                    ...
                    <span class="detail-value">{{ number_format($attendance->overtime_hours, 2) }} hours</span>
                </div>
            @endif
        ...
        @if($notificationType === 'late_check_in' || $notificationType === 'early_checkout')
            <div class="alert alert-warning">
        ...
        <div style="text-align: center; margin-top: 30px;">
            <a href="{{ url('/attendance/dashboard') }}" class="btn">View Attendance Dashboard</a>
        </div>
    </div>
