                            ...
                            <table class="table table-hover table-striped" id="accountsTable">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge bg-{{ $quotation->invoice_status_color }} me-1" id="invoice-status-{{ $quotation->id }}">
                                                <i class="fas fa-{{ $quotation->invoice_status_icon }}"></i>
                                                {{ ucfirst($quotation->invoice_status ?: 'pending') }}
                                            </span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <form action="{{ route('accounts.update-payment', $quotation->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('PUT')
                                                <select name="payment_status" class="form-select form-select-sm" onchange="this.form.submit()">
                                                    @foreach(\App\Models\Quotation::getPaymentStatuses() as $key => $value)
                                                        <option value="{{ $key }}" {{ $quotation->payment_status == $key ? 'selected' : '' }}>
                                                            {{ $value }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="small text-muted mt-1">
                                                {{ $quotation->payment_updated_at->format('M d, Y h:i A') }}
                                            </div>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input customer-panel-toggle" 
                                                       type="checkbox" 
                                                       id="customerPanel{{ $quotation->id }}"
                                                       data-quotation-id="{{ $quotation->id }}"
                                                       {{ $quotation->customer_panel ? 'checked' : '' }}>
                                                <label class="form-check-label" for="customerPanel{{ $quotation->id }}">
                                                    {{ $quotation->customer_panel ? 'Enabled' : 'Disabled' }}
                                                </label>
                                            </div>
                                            @if($quotation->customer_panel)
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="{{ route('accounts.create-invoice', $quotation->id) }}" class="btn btn-sm btn-success" title="Generate GST Invoice">
                                                    <i class="fas fa-file-invoice"></i> Create Invoice
                                                </a>
                                                <a href="{{ route('invoices.management', $quotation->id) }}" class="btn btn-sm btn-info" title="Invoice Management">
                                                    <i class="fas fa-tasks"></i> Invoice Management
                                                </a>
                                                <a href="{{ route('quotations.edit', $quotation->id) }}" class="btn btn-sm btn-primary" title="Edit Quotation">
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="{{ route('quotations.show', $quotation->id) }}" class="btn btn-sm btn-info" title="View Details">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="{{ route('quotations.pdf', $quotation->id) }}" class="btn btn-sm btn-secondary" title="Download PDF">
                                                    <i class="fas fa-download"></i>
                                                </a>
                                                @if($quotation->lead_id)
                                                <button class="btn btn-sm btn-warning" onclick="moveQuotationToLead({{ $quotation->id }}, {{ $quotation->lead_id }})" title="Move back to Leads Qualified">
                            ...
                            <table class="table table-hover table-striped" id="qualifiedLeadsTable">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($lead->budget)
                                                <span class="badge bg-info">{{ number_format($lead->budget, 2) }}</span>
                                            @else
                                        ...
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input customer-panel-toggle" 
                                                       type="checkbox" 
                                                       id="leadCustomerPanel{{ $lead->id }}"
                                                       data-lead-id="{{ $lead->id }}"
                                                       {{ $lead->customer_panel ? 'checked' : '' }}>
                                                <label class="form-check-label" for="leadCustomerPanel{{ $lead->id }}">
                                                    {{ $lead->customer_panel ? 'Enabled' : 'Disabled' }}
                                                </label>
                                            </div>
                                            @if($lead->customer_panel)
                                                <div class="small text-success mt-1">
                                                    <i class="fas fa-check-circle"></i> Customer can login
                                                </div>
                                            @else
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                ...
                                                @if(!checkLeadInQuotations($lead->id))
                                                <button class="btn btn-sm btn-success" onclick="moveLeadToQuotation({{ $lead->id }})" title="Move to Account Management">
