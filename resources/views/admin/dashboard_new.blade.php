...
<head>
    ...
    <style>
        ...
        *::after {
        ...
        /* Mobile display stability fixes */
...
<body>
            ...
            @php
                ...
                function getDateGroupLabel($taskDate) {
                    ...
                    if ($task->eq($today)) {
                        ...
                        return $task->format('M j' . ($task->year != now()->year ? ', Y' : ''));
                    }
                }
                        ...
                        <article class="task-card" data-task-id="{{ $task->id }}" data-aos="fade-up" data-aos-delay="{{ $delayCounter }}">
                            <header class="task-header">
                                ...
                                <time class="task-date" datetime="{{ $task->task_date->format('Y-m-d H:i') }}" aria-label="Task date">
                                    ...
                                    <span>{{ $task->task_date->format('g:i A') }}</span>
                                </time>
                            </header>
    ...
    <script>
        ...
        // Modern loading states with mobile optimization
        function showLoading() {
            ...
            if (container) {
                // Don't immediately hide container on mobile to prevent flickering
        ...
        function hideLoading() {
            ...
            if (container) {
                ...
                // Delay AOS refresh to prevent mobile rendering issues
                setTimeout(() => {
                    AOS.refresh();
                }, 100);
            }
        ...
        // Apply filters with modern async/await and mobile optimization
        async function applyFilters() {
            try {
                ...
                if (filterBtn && window.innerWidth <= 768) {
                    ...
                    filterBtn.innerHTML = '<i class="bi bi-hourglass-split"></i> <span>Filtering...</span>';
                }
                ...
                if (filterBtn && window.innerWidth <= 768) {
                    ...
                    filterBtn.innerHTML = '<i class="bi bi-search"></i> <span>Apply Filters</span>';
                }
            }
        }
        ...
        function getDateGroupLabel(taskDate) {
            ...
            if (taskStart.getTime() === todayStart.getTime()) {
                ...
                return task.toLocaleDateString('en-US', { 
                    month: 'short', 
                    day: 'numeric', 
                    year: task.getFullYear() !== today.getFullYear() ? 'numeric' : undefined 
                });
            }
        }

        // Render tasks with date grouping and mobile optimization
        function renderTasks(tasks) {
            const container = document.getElementById('taskContainer');
            
            if (!container) return;
            
            // Mobile-specific: ensure container is visible and properly styled
            ...
            if (tasks.length === 0) {
                ...
                // Delay AOS refresh on mobile to prevent flickering
                if (isMobile) {
                        ...
                        <article class="task-card" data-task-id="${task.id}" data-aos="fade-up" data-aos-delay="${delayCounter}">
                            <header class="task-header">
                                ...
                                <time class="task-date" datetime="${new Date(task.task_date).toISOString()}">
                                    <i class="bi bi-calendar" aria-hidden="true"></i>
                                    <span>${new Date(task.task_date).toLocaleString('en-US', { 
                                        hour: 'numeric', 
                                        minute: '2-digit',
                                        hour12: true 
                                    })}</span>
            ...
            // On mobile, set innerHTML with a delay to prevent flickering
            if (isMobile) {
                // Clear container first
                container.innerHTML = '';
                
                // Then add content with a small delay
                setTimeout(() => {
