...
@section('page-title', 'Due Date Management')
...
<div class="container-fluid">
        ...
        <div class="col-md-2">
            <div class="card stats-card text-center h-100">
                <div class="card-body">
                    ...
                    <h3 class="text-danger">{{ $leads->getCollection()->where('urgency_status.status', 'overdue')->count() }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-2">
            <div class="card stats-card text-center h-100">
                <div class="card-body">
                    ...
                    <h3 class="text-warning">{{ $leads->getCollection()->where('urgency_status.status', 'today')->count() }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-2">
            <div class="card stats-card text-center h-100">
                <div class="card-body">
                    ...
                    <h3 class="text-info">{{ $leads->getCollection()->where('urgency_status.status', 'urgent')->count() }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-2">
            <div class="card stats-card text-center h-100">
                <div class="card-body">
                    ...
                    <h3 class="text-primary">{{ $leads->getCollection()->where('urgency_status.status', 'soon')->count() }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-2">
            <div class="card stats-card text-center h-100">
                <div class="card-body">
                    ...
                    <h3 class="text-success">{{ $leads->getCollection()->where('urgency_status.status', 'normal')->count() }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-2">
            <div class="card stats-card text-center h-100">
                <div class="card-body">
                    ...
                    <h3 class="text-secondary">{{ $leads->total() }}</h3>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('duedate.index') }}">
                <div class="row">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-2">
                        ...
                        <select class="form-select" id="urgency" name="urgency">
                            ...
                            <option value="overdue" {{ request('urgency') == 'overdue' ? 'selected' : '' }}>Overdue</option>
                            <option value="today" {{ request('urgency') == 'today' ? 'selected' : '' }}>Due Today</option>
                            <option value="this_week" {{ request('urgency') == 'this_week' ? 'selected' : '' }}>This Week</option>
                            <option value="this_month" {{ request('urgency') == 'this_month' ? 'selected' : '' }}>This Month</option>
                            <option value="next_month" {{ request('urgency') == 'next_month' ? 'selected' : '' }}>Next Month</option>
                        </select>
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('duedate.index') }}" class="btn btn-secondary">
        ...
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-3">
                    <span class="badge bg-info">{{ $leads->total() }} leads with due dates</span>
                            ...
                            <tr>
                                <td>
                                    <input type="checkbox" class="form-check-input lead-checkbox" 
                                           value="{{ $lead->id }}" data-lead-name="{{ $lead->name }}">
                                </td>
                                <td>
                                ...
                                <td>
                                    <strong>{{ $lead->due_date->format('M d, Y') }}</strong>
                                    <br><small class="text-muted">{{ $lead->due_date->format('l') }}</small>
                                </td>
                                <td>
                                    <span class="badge urgency-{{ $lead->urgency_status['status'] }}">
                                        {{ $lead->urgency_status['days_text'] }}
                                    </span>
                                </td>
                                <td>
                                    <span class="badge bg-{{ $lead->urgency_status['color'] }}">
                                        {{ $lead->urgency_status['label'] }}
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        ...
                                        <a href="{{ route('leads.edit.new', $lead->id) }}" class="btn btn-sm btn-info" title="Edit Lead">
