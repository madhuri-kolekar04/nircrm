                        ...
                        @foreach($tasksInGroup as $task)
                            <div class="task-card" data-task-id="{{ $task->id }}" data-aos="fade-up" data-aos-delay="{{ $delayCounter }}">
                                <div class="task-header">
                                    ...
                                    <div class="task-date">
                                        <i class="bi bi-calendar"></i>
                                        <span>{{ \Carbon\Carbon::parse($task->task_date)->format('g:i A') }}</span>
                                    </div>
                                </div>
    ...
    <script>
        ...
        function updateTaskList(tasks) {
            ...
            Object.keys(groupedTasks).forEach(date => {
                html += `<div class="task-group" data-aos="fade-up" data-aos-delay="${delayCounter}"><h3 class="group-title"><i class="bi bi-calendar3 me-2"></i>${date}</h3><div class="task-list">`;
                
                groupedTasks[date].forEach(task => {
                    const statusClass = task.status.replace('_', '-');
                    const statusLabel = task.status.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase());
                    
                    html += `<div class="task-card" data-aos="fade-up" data-aos-delay="${delayCounter + 100}"><div class="task-header"><div class="task-number"><span class="task-date">${new Date(task.task_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span><span class="number">#${task.task_number}</span></div><div class="task-status status-${statusClass}"><i class="bi bi-circle-fill me-1"></i>${statusLabel}</div></div><div class="task-body"><div class="task-description"><p>${task.task_description}</p></div><div class="task-client"><i class="bi bi-briefcase me-1"></i><span>${task.client_project_name}</span></div></div><div class="task-footer"><div class="task-time"><i class="bi bi-clock me-1"></i><span>${new Date(task.task_date).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span></div><div class="task-actions"><button class="btn-action btn-edit" onclick="editTask(${task.id})" title="Edit"><i class="bi bi-pencil"></i></button><button class="btn-action btn-delete" onclick="deleteTask(${task.id})" title="Delete"><i class="bi bi-trash"></i></button></div></div></div>`;
                });
